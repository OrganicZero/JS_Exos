...
console.log("Est-ce que tous les ont été empruntés au moins une fois ?");

function hasBeenRented(book) {
  for (let i = 0; i < book.length; i++ ) {
    if (book[i].rented === 0) {
      return "Il y a au moins un livre qui n'a jamais été emprunté.";
      break;
    }
  }
  return "Tous les livres ont déjà été emprunté."
}

console.log(hasBeenRented(books));
...
for (let index in books) {
  if (books[index].id === 133712) {
    console.log(`${books[index].title} a été supprimé.`)
